...
def read_line_or_begin(fd) -> str:
    """Функция для чтения более короткого файла"""
    text = fd.readline()
    if text == '':
        fd.seek(0)
        text = fd.readline()
    return text[:-1]  # отрезает переход строки на новую
    ...
    with open(file_numbers, 'r', encoding='utf-8') as f_num:
        with open(file_names, 'r', encoding='utf-8') as f_names:
            with open(file_res, 'w', encoding='utf-8') as f_res:
                ...
                length = max(length1, length2) # выяснили, какой файл длиннее
                for i in range(length):
                    line_num = read_line_or_begin(f_num) # читаем строку с числами
                    line_name = read_line_or_begin(f_names) # читаем строку с именами
                    a, b = line_num.split('|') # разделяем список на два элемента
                    a = int(a)
                    b = float(b)
                    res = a * b
                    if res < 0:
