...
def generate_files(extension:str,
    ...
    if not os.path.exists(directory) or not os.path.isdir(directory): # проверка существования директории
        os.mkdir(directory)
    for i in range(num_files):
        ...
        while True:
            try: # Проверка существования файла с режимом х
                with open(f'{directory}/{filename}.{extension}', 'x', encoding='utf-8') as f:
                    f.write(text)
            except: # Если файл есть, то сгенерировать новое имя
                filename = generate_text(name_length)
            else: # Если файла нет, то выходим из цикла
                break


if __name__ == '__main__':
    generate_files('rnd', 'files')
"""Создали папку files с файлами с расширением rnd"""
